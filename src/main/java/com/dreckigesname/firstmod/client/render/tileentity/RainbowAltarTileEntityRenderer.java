package com.dreckigesname.firstmod.client.render.tileentity;
...
public class RainbowAltarTileEntityRenderer extends TileEntityRenderer<RainbowAltarTileEntity> {

	private Minecraft mc = Minecraft.getInstance();
	
	private int renderStateCount = 0;

	public RainbowAltarTileEntityRenderer(TileEntityRendererDispatcher rendererDispatcherIn) {
		super(rendererDispatcherIn);
	}

	@Override
	public void render(RainbowAltarTileEntity te, float partialTicks, MatrixStack matrixStackIn,
		...
		renderItem(te.getItem(), new double[] { 0.5d, 1d, 0.5d },
				Vector3f.YP.rotationDegrees(renderStateCount), matrixStackIn, bufferIn, partialTicks,
				combinedOverlayIn, lightLevel, 1f);
		
		ITextComponent label = te.getItem().hasCustomHoverName() ? te.getItem().getDisplayName() : new TranslationTextComponent(te.getItem().getDescriptionId());
